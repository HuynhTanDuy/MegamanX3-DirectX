...
QuadTree::QuadTree(int id,int level, RECT bound)
{
    //chia lam 4
	this->mId = id;
    this->Bound = bound;
...
{
    int index = getIndex(entity->GetBound()); // xac dinh vi tri cua Entity
...
void QuadTree::split()
{
    ...
    Nodes[0] = new QuadTree(mId * 10 + 1,mLevel + 1, bound);
    ...
    bound.top = Bound.top;
    bound.bottom = bound.top + height;
    Nodes[1] = new QuadTree(mId * 10 + 2, mLevel + 1, bound);
    ...
    bound.bottom = bound.top + height;
    Nodes[2] = new QuadTree(mId * 10 + 3, mLevel + 1, bound);
    ...
    bound.top = Bound.top + height;
    bound.bottom = bound.top + height;
    Nodes[3] = new QuadTree(mId * 10 + 4, mLevel + 1, bound);
}
...
std::vector<Entity*> QuadTree::getmListEntity()
{
	return mListEntity;
}
...
void QuadTree::getEntitiesCollideAble(std::vector<Entity*> &entitiesOut, Entity *entity)
{
    ...
    if (index != -1)
    {
        //nhung Entity o day se la nam tren 2 node con nen ta cung se lay de set va cham
        for (auto child : mListEntity)
        {
            entitiesOut.push_back(child);
        }
